  ...
  try {
    ...
    const nextMatch = matches.find((match) => {
      const matchDate = new Date(
        match.match.scheduled_time || match.match.created_at,
      );
      ...
      return (
        matchDate > now &&
        (match.match.player1_id === teamID || match.match.player2_id === teamID)
      );
    });
    console.log(matches);
...
export default function DomdComp() {
  ...
  const changeTeam = async (event) => {
    ...
    const teamHasGame = doesTeamHaveGame(
      allGamesOfTournament,
      teamsID[teamName],
    );
    if (teamHasGame) {
      setTeamHasGame(true);
      ...
      const dartBoardNumb = await getDartBoardNumb(
        tournamentID,
        allGamesOfTournament,
        nextGame,
      );
    ...
    async function getAllTeams() {
      ...
      try {
        ...
        for (let team of teams) {
          const teamCapitlizedName =
            team.participant.name.charAt(0).toUpperCase() +
            team.participant.name.slice(1);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Typography
            ...
            sx={{
              textAlign: "center",
              display: displayTeam ? "none" : "block",
            }}
          >
            Ta reda på när ditt lag spelar under tävlingen!
          </Typography>
          <Typography
            variant="h4"
            sx={{
              textAlign: "center",
              display:
                displayTeam && teamHasGame && !isFinal ? "block" : "none",
            }}
          >
          ...
          >
            Lag {selectedTeam} har inga matcher att spela! Njut av en kall
            DömD-IPA!
          </Typography>
        </Paper>
      </Box>
      ...
      >
        <Typography>
          För mer info om DÖMD 2024 kontakta sekretariatet eller gå till{" "}
          <Link href={`https://challonge.com/domd2024`} target="_blank">
            CHALLONGE
          </Link>
        </Typography>
      </Box>
